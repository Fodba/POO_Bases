...
class Employe
{
    ...
    public function __construct($default = false){
        self::$nbrEmploye++;
        if($default){
            $this->setNom("Nom ".self::$nbrEmploye);
            $this->_prenom = "Prenom ".self::$nbrEmploye;
            $this->setDateEmbauche(rand(1,28)."/".rand(1,12)."/".rand(2000,2018));
            $this->_fonction = "fonction lambda ".self::$nbrEmploye;
            $this->_salaire = rand(10,80); // Ce devrait être 30 car le salaire est exprimé en K€
            $this->_service = "Commercial ".self::$nbrEmploye;
        }
    }
